...
public class LuceneSearch1 {
	...
	public static void main(String[] args) throws FileNotFoundException, IOException {
		...
		File ifile= new File(indexDirectory);
		File dfile= new File(dataDirectory);
		if(!ifile.exists()) ifile.createNewFile();
		if(!dfile.exists()) dfile.createNewFile();
		LuceneSearch1 luceneDemo = new LuceneSearch1();		
		//create Lucene index
	...
	private void rangeQueryExample(){
	    ...
	    Query query = TermRangeQuery.newStringRange("date", "20090601", "20090606", true,true);
	    showSearchResults(query);
	}
	...
	private void deletDocumentFromIndex(){
		try {
			...
			//IndexReader indexReader = IndexReader.open(ireader);
	...
	void sortBySenderExample(){
		 ...
		 */
		SortField sortField = new SortField("sender",SortField.Type.STRING, true);	
